...
export const NewBatchModal = ({ isOpen, onClose }) => {
  const { createProductBatch, employees, orderBookings } = useApp();
    ...
    >
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div>
          ...
          <select className="form-select" value={bookingId} onChange={(e) => setBookingId(e.target.value)}>
            <option value="">Standalone showroom / inventory batch</option>
            {orderBookings.filter((booking) => booking.status !== 'Delivered').map((booking) => (
              <option key={booking.id} value={booking.id}>
                {booking.bookingNo} - {booking.customerName} - {booking.garmentType}
              </option>
            ))}
          </select>
        </div>
